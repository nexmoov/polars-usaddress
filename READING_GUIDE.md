...
tokenizer, feature extraction, and inference engine are reimplemented.
...
### `src/tokenize.rs` (84 lines)

The smallest file and a gentle warm-up. It splits an address string into
tokens with one regex, and normalizes HTML ampersand entities with another.
...
- `tokens_to_features` combines every token's own features with its
  ...
  attribute list the CRF model scores against.
- Everything below the `// id-based path` marker is a second, faster
  implementation of the same thing, which section 3 explains.
...
joined into one string) and classifies the address type. The `_confidence`
...
### `tests/` and `examples/bench_split.rs`

`tests/fixtures.json` is a shared corpus of real addresses (including some
deliberately ambiguous ones) that several files' tests load via
`include_str!`. `tests/parity.rs` (not covered above -- worth a look on its
own) checks this crate's output against upstream Python's output directly.
`examples/bench_split.rs` is a standalone binary (`cargo run --example
bench_split`) for measuring where time actually goes at runtime; its own
doc comment explains how to run it and how to compare against the Python
benchmark notebook.
...
```sh
# Run every unit test, across every file in src/
cargo test
